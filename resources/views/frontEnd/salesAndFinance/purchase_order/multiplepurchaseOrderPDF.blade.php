								...
								<tr>
									<td>
										<h3 style="margin-bottom: 0; font-size: 24px; margin-top: 0; padding-top: 8px; padding-bottom: 8px;">
											<strong>Purchase Order - {{$po_val->purchase_order_ref}}</strong>
										</h3>
										<p style="margin-bottom: 0; font-size: 18px; margin-top: 0; padding-top: 8px; padding-bottom: 8px;">
											<strong>Date: {{date('d/m/Y',strtotime($po_val->purchase_date))}}</strong>
										</p>
									</td>
								</tr>
							</table>
					...
					<tr style="border-top: 1px solid #000; border-bottom: 1px solid #000;">
							...
							<table cellpadding="0" cellspacing="0" width="100%"
								...
								<thead>
									<tr style="font-size: 14px;">
										<th>
											<p style="margin: 0; text-align: left;">{{$po_val->suppliers->name}}</p>
										</th>
									</tr>
								</thead>
								<tbody>
									<tr >
										<td style="font-size: 13px;">
											<p style="margin: 5px 0;">{{$po_val->suppliers->contact_name}} </p>
											<p style="margin: 5px 0;">{{$po_val->suppliers->address}} </p>
											<p style="margin: 5px 0;">{{$po_val->suppliers->city}}</p>
						...
						<td style="width: 40%; float: left; padding-top: 10px; padding-bottom: 10px; vertical-align: baseline;">
							<table cellpadding="0" cellspacing="0" width="100%"
								...
								<tbody>
									<tr>
										<?php if ($po_val->site_id == 0) {
											$site_detail = App\Models\Customer::find($po_val->customer_id);
										...
										<?php } else {
											$site_detail = App\Models\Constructor_customer_site::find($po_val->site_id);
										?>
											<td style="font-size: 13px;">
												<p style="margin: 5px 0;">{{$site_detail->address ?? ""}} </p>
												<p style="margin: 5px 0;">{{$site_detail->city ?? ""}} </p>
							...
							<div class="main-table">
								<table cellpadding="0" cellspacing="0" width="100%" class="table" style=" margin-top:20px;">
									<tbody>
										...
										$sub_total_amount = 0;
										$vat_amount = 0;
										foreach ($po_val->purchaseOrderProducts as $key => $val) {
								...
								<table cellpadding="0" cellspacing="0" width="100%" class="table">
									<tbody style=" width: 100%; float: left;">
										...
										<tr class="product_details">
											<th colspan="5" style="width: 60%;"></th>
											<th colspan="1" style="width: 20%; text-align: right;">
												<p class="totlePrice" style="margin: 0; padding-bottom: 20px; font-size: 14px;">Total (inc. VAT)</p>
											</th>
											<?php
											$total_amount = $sub_total_amount + $vat_amount;
											?>
											<th colspan="1" style="width: 20%; text-align: right;">
												<p class="totlePrice" style="margin: 0; padding-bottom: 20px; font-size: 14px;">£{{$total_amount}}</p>
											</th>
										</tr>
										<tr class="product_details" style="border-top: 1px solid #000; margin-top: 12px;">
											<th colspan="5"></th>
											<th colspan="2">
