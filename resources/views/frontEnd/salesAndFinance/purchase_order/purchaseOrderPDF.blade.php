<!DOCTYPE html>
<html>

<head>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="shortcut icon" type="image/x-icon" href="{{ url('public/images/favicon.ico') }}">
	<title>Purchase Order invoice</title>
	<link href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;1,300;1,400;1,500;1,700&display=swap" rel="stylesheet">
	<style type="text/css">
		*{
			padding: 0;
			margin: 0;
		}
		body {
			font-family: 'Roboto', sans-serif;
			background: #fff;
		}

		.main-center-box {
			width: 800px;
			margin: 0 auto;
		}

		@media only screen and (max-width: 600px) {
			.main-center-box {
				width: 100%;
				margin: 0 auto;
			}
		}

		.product_details td {
			padding: 6px 0;
			text-align: center;
			border: 1px solid #eee;
		}
		.totlePrice{
			padding-top: 10px;
			white-space: nowrap;
            text-align: end;
		}
		.pagination{
			display: flex
;
    list-style: none;
    justify-content: flex-end;
    font-size: 14px;
    font-weight: 400;
    padding-top: 12px;
    border-top: 1px solid #eee;
    margin-top: 12px;
		}
		ul.pagination li {
    padding: 0 10px;
}
	</style>
</head>

<body>
	<div class="main-center-box">
		<div style="background:#f2f3f5;padding:12px;float:left; width: 100%;">
			<table table="border" cellpadding="0" cellspacing="0" width="100%"
				style="background:#fafafa;padding: 12px;">
				<tbody style="padding-bottom: 40px;">
					<tr>
						<!-- <td style="width: 100%;float: left; text-align: center;">
							<h4 style="margin-top: 0;margin-bottom: 0;font-size: 22px;">Order Invoice</h4>
						</td> -->
						<td style="width: 100%;float: left;">
							<table cellpadding="0" cellspacing="0"
								style="text-align: left;padding: 20px 0;float:left ;width: 40%;">
								<tr>
									<td style="font-size:18px;font-weight:600;color:#000;">
                                        <img src="{{url('public/images/ewm_logo.png')}}" style="width:180px;">
                                    </td>
								</tr>
								<tr>
									<td>
										<h3 style="margin-bottom:0;font-size:24px;margin-top:0; padding: 8px;">
											<strong>PurchaseOrder - PO-0005 </strong> </h3>
										<p style="margin-bottom:0;font-size:18px;margin-top:0; padding: 8px;">
											<strong>Date: 06/01/2025 </strong> </p>
									</td>
								</tr>
							</table>
							<table cellpadding="0" cellspacing="0"
								style="padding: 10px 0;width: 60%;text-align: right;">
								<tr>
									<td
										style="font-size:18px;font-weight:600;color:#000;padding-top:15px;margin-bottom: 0">
										Blueprint Contruction</td>
								</tr>
								<tr>
									<td>
										<p style="margin:2px 0;font-size:14px;">13 POLAND STREET, LONDON</p>
										<p style="margin:2px 0;font-size:14px;">LONDON</p>
										<p style="margin:2px 0;font-size:14px;">UK</p>
										<p style="margin:2px 0;font-size:14px;">W1F 8QB</p>
										<p style="margin:2px 0;font-size:14px;"><strong>Tel:</strong> +447488803756</p>
										<p style="margin:2px 0;font-size:14px;"><strong>Email:</strong>
											geo@flash-interiors.co.uk</p>
									</td>
								</tr>
							</table>
							<table cellpadding="0" cellspacing="0" width="100%"
								style="text-align:left;padding-top:20px;border-top:1px solid #000;border-bottom:1px solid #000; padding-bottom: 20px;">
								<thead>
									<tr style="font-size: 14px;">
										<th style="width: 50%;">
											<p style="margin: 0;">Supplier1</p>
										</th>
										<th style="width: 50%;">
											<p style="margin: 0;">Site / Delivry Address</p>
										</th>
									</tr>
								</thead>
								<tbody>
									<tr>
										<td style="font-size: 13px;">
											<p style="margin: 5px 0;">Mahendra Kumar </p>
											<p style="margin: 5px 0;">13 POLAND STREET, LONDON</p>
											<p style="margin: 5px 0;">London</p>
											<p style="margin: 5px 0;">GB</p>
											<p style="margin: 5px 0;">SW7 1EE</p>
										</td>

										<td style="font-size: 13px;">
											<p style="margin: 5px 0;">Mahendra </p>
											<p style="margin: 5px 0;">Home No.324 </p>
											<p style="margin: 0;font-size: 13px;">4 MONTPELIER STREET, KINGHTSBRIDGE</p>
											<p style="margin: 5px 0;">London</p>
											<p style="margin: 5px 0;">SW7 1EE</p>
										</td>
									</tr>

								</tbody>
							</table>
							<table cellpadding="0" cellspacing="0" width="100%" class="table">
								<tbody>
									<tr style="font-size: 14px;background-color: #e5e5e5; text-align: center;">
										<td style="width: 20px;">
											<p style="padding: 6px 4px;"><strong>#</strong></p>
										</td>
										<td style="width: 60px;">
											<p style="padding: 6px 4px;"><strong>Product</strong></p>
										</td>
										<td style="width: 90px;">
											<p style="padding: 6px 4px;"><strong>Description</strong></p>
										</td>
										<td style="width: 50px;">
											<p style="padding: 6px 4px;"><strong>Qty</strong></p>
										</td>
										<td style="width: 50px;">
											<p style="padding: 6px 4px;"><strong>Unit Price</strong></p>
										</td>
										<td style="width: 60px;">
											<p style="padding: 6px 4px;"><strong>VAT(%)</strong></p>
										</td>
										<td style="width: 70px;">
											<p style="padding: 6px 4px;"><strong>Net Amount</strong></p>
										</td>
									</tr>
									<tr class="product_details">
										<td>
											<p style="margin: 0;font-size: 14px;">01</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">Product1</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">This is besic project </p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">2</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">$200.00</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">5</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">$200.00</p>
										</td>
									</tr>
									<tr class="product_details">
										<td>
											<p style="margin: 0;font-size: 14px;">02</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">Product1</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">This is besic project </p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">2</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">$200.00</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">5</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">$200.00</p>
										</td>
									</tr>
									<tr class="product_details">
										<td>
											<p style="margin: 0;font-size: 14px;">03</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">Product1</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">This is besic project </p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">2</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">$200.00</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">5</p>
										</td>
										<td>
											<p style="margin: 0;font-size: 14px;">$200.00</p>
										</td>
									</tr>
								
							
								</tbody>
								<tfoot>
									<tr class="product_details">
										<th colspan="5"></th>
										<th> <p class="totlePrice" style="margin: 0;font-size: 14px;font-weight: 400;">Sub Total (exc. VAT) </p> </th>
										<th><p class="totlePrice" style="margin: 0;font-size: 14px;font-weight: 400;">£00.00</p> </th>
									</tr>
									<tr class="product_details">
										<th colspan="5"></th>
										<th> <p class="totlePrice" style="margin: 0;font-size: 14px;font-weight: 400;">VAT</p> </th>
										<th><p class="totlePrice" style="margin: 0;font-size: 14px;font-weight: 400;">£00.00</p> </th>
									</tr>
									<tr class="product_details">
										<th colspan="5"></th>
										<th> <p class="totlePrice" style="margin: 0;font-size: 14px;">Total (inc. VAT)</p> </th>
										<th><p class="totlePrice" style="margin: 0;font-size: 14px;">£5500.00</p> </th>
									</tr>
									<tr class="product_details">
										<th  colspan="7">
										   <ul class="pagination">
												<li>Page</li>
												<li> 1 </li>
												<li> of </li>
												<li> 1</li>
										   </ul>	
										</th>
									</tr>
								</tfoot>
							</table>
						</td>
					</tr>
				</tbody>
			</table>
</body>

</html>