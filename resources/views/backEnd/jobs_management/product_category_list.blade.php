...
<?php
    $page_url = url('admin/product_category');
?>
                        ...
                        <div class="adv-table editable-table ">
                         <div class="row"> 
                          <div class="col-lg-6">  
                            <div class="clearfix">
                                <div class="btn-group">
                                    <a href="{{url('admin/product_category_add')}}">
                                        <button id="editable-sample_new" class="btn btn-primary">
                                            Add Product Catetgory <i class="fa fa-plus"></i>
                                        </button>
                                    </a>    
                                </div>
                                ...
                                <table class="table table-striped table-hover table-bordered" id="editable-sample">
                                    ...
                                    <tbody>
                                        ...
                                        if(count($cat) == 0){ ?>
                                            <?php
                                                echo '<tr style="text-align:center">
                                                      <td colspan="7">No Product Category found.</td>
                                                      </tr>';
                                            ?>
                                        <?php 
                                        } 
                                        ...
                                        else
                                        {
                                            ...
                                            {  
                                                $product_count=App\Models\Product::where('cat_id',$val->id)->count();
                                            ?>
                                        ...
                                        <tr >
                                            ...
                                            <td class="transform-none" style="text-transform: none;">{{ $val->full_category }}</td>
                                          
                                            <td>{{$product_count}}</td>
                                            <td>{{ $val->children_count }}</td>
                                            <td>
                                                @if($val->status == 1)
                                                    <a href="javascript:" onclick="status_change('{{base64_encode($val->id)}}',0)" class="btn btn-success">Active</a>
                                                @else
                                                <a href="javascript:" class="btn btn-danger" onclick="status_change('{{base64_encode($val->id)}}',1)">In-Active</a>
                                                @endif
                                            </td>
                                            <td class="action-icn">
                                                <a href="{{ url('admin/product_category_add?key=')}}{{base64_encode($val->id)}}" class="edit"><span style= "font-size: 13px; color: #000;"><span style= "color: #000"><i data-toggle="tooltip" title="Edit" class="fa fa-edit fa-lg"></i></span></a> &nbsp &nbsp &nbsp 
...
<script>
    function status_change(id,status){
        ...
        var token='<?php echo csrf_token();?>'
            $.ajax({  
                type:"POST",
                url:"{{url('admin/product_cat_status_change')}}",
    ...
    function delete_job(id){
       if(confirm("Do you want to delete it ?")){
        ...
        var token='<?php echo csrf_token();?>'
            $.ajax({  
                type:"POST",
                url:"{{url('admin/product_cat_delete')}}",
