...
<?php
    $page_url = url('admin/job_appointment_type');
?>
                        ...
                        <div class="adv-table editable-table ">
                         <div class="row"> 
                          <div class="col-lg-6">  
                            <div class="clearfix">
                                <div class="btn-group">
                                    <a href="{{url('admin/job_appointment_type_add')}}">
                                        <button id="editable-sample_new" class="btn btn-primary">
                                            Add Job Appoinitment Type <i class="fa fa-plus"></i>
                                        </button>
                                    </a>    
                                </div>
                           ...
                           <div class="col-lg-6">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-hover table-bordered" id="editable-sample">
                                    ...
                                    <tbody>
                                        <?php
                                        if(count($appointtype) == 0){ ?>
                                            <?php
                                                echo '<tr style="text-align:center">
                                                      <td colspan="8">No Job Type found.</td>
                                                      </tr>';
                                            ?>
                                        <?php 
                                        } 
                                        ...
                                        {
                                            foreach($appointtype as $key => $val) 
                                            {  ?>

                                        <tr >
                                            <td>{{++$key}}</td>
                                            <td class="user_name">{{ ucfirst($val->name) }}</td>
                                            <td class="transform-none" style="text-transform: none;">No</td>
                                            <td>No</td>
                                            <td><?php echo ($val->auth==1)?"Yes":"No";?></td>
                                            <td>{{$val->hours}} : {{$val->minute}}</td>
                                            <td>
                                                @if($val->status == 1)
                                            ...
                                            <td class="action-icn">
                                                <a href="{{ url('admin/job_appointment_type_add?key=')}}{{base64_encode($val->id)}}" class="edit"><span style= "font-size: 13px; color: #000;"><span style= "color: #000"><i data-toggle="tooltip" title="Edit" class="fa fa-edit fa-lg"></i></span></a> &nbsp &nbsp &nbsp 
...
<script>
    function status_change(id,status){
        ...
        var token='<?php echo csrf_token();?>'
            $.ajax({  
                type:"POST",
                url:"{{url('admin/job_appointment_type_status_change')}}",
    ...
    function delete_job(id){
       if(confirm("Do you want to delete it ?")){
        ...
        var token='<?php echo csrf_token();?>'
            $.ajax({  
                type:"POST",
                url:"{{url('admin/job_appointment_type_delete')}}",
