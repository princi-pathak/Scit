...
<?php
    $page_url = url('admin/account_list');
?>
                        ...
                        <div class="adv-table editable-table ">
                         <div class="row"> 
                          <div class="col-lg-6">  
                            <div class="clearfix">
                                <div class="btn-group">
                                    <a href="{{url('admin/account_code_add')}}">
                                        <button id="editable-sample_new" class="btn btn-primary">
                                            Departmental Code - Add <i class="fa fa-plus"></i>
                                        </button>
                                    </a>    
                                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-hover table-bordered" id="editable-sample">
                                    ...
                                    <tbody>
                                        ...
                                        if(count($acc_code) == 0){ ?>
                                            <?php
                                                echo '<tr style="text-align:center">
                                                      <td colspan="5">No Account Code found.</td>
                                                      </tr>';
                                            ?>
                                        <?php 
                                        } 
                                        ...
                                        {
                                            foreach($acc_code as $key => $val) 
                                            {  ?>

                                        <tr >
                                            <td class="user_name">{{ ++$key }}</td>
                                            <td class="transform-none" style="text-transform: none;">{{ucfirst($val->name)}}</td>
                                            ...
                                            <td class="action-icn">
                                                <a href="{{ url('admin/account_code_add?key=')}}{{base64_encode($val->id)}}" class="edit"><span style= "font-size: 13px; color: #000;"><span style= "color: #000"><i data-toggle="tooltip" title="Edit" class="fa fa-edit fa-lg"></i></span></a> &nbsp &nbsp &nbsp 
...
<script>
    function status_change(id,status){
        ...
        var token='<?php echo csrf_token();?>'
            $.ajax({  
                type:"POST",
                url:"{{url('admin/account_status_change')}}",
