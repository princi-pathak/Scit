...
<?php
    $page_url = url('admin/product_list');
?>
                                ...
                                <table class="table table-striped table-hover table-bordered" id="editable-sample">
                                    ...
                                    <tbody>
                                        ...
                                        if(count($product) == 0){ ?>
                                            <?php
                                                echo '<tr style="text-align:center">
                                                      <td colspan="7">No Job Type found.</td>
                                                      </tr>';
                                            ?>
                                        <?php 
                                        } 
                                        ...
                                        else
                                        {
                                            ...
                                            {  
                                                $category_details=App\Models\Product_category::find($val->cat_id);
                                                ?>

                                        <tr >
                                            <td class="user_name">{{ ucfirst($val->product_name) }}</td>
                                            <td class="transform-none" style="text-transform: none;">{{$category_details->name}}</td>
                                            <td>{{$val->price}}</td>
                                            <td>{{$val->qty}}</td>
                                            ...
                                            <td class="action-icn">
                                                <a href="{{ url('admin/product_add?key=')}}{{base64_encode($val->id)}}" class="edit"><span style= "font-size: 13px; color: #000;"><span style= "color: #000"><i data-toggle="tooltip" title="Edit" class="fa fa-edit fa-lg"></i></span></a> &nbsp &nbsp &nbsp 
...
<script>
    function status_change(id,status){
        ...
        var token='<?php echo csrf_token();?>'
            $.ajax({  
                type:"POST",
                url:"{{url('admin/product_status_change')}}",
