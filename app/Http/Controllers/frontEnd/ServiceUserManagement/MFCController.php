...
class MFCController extends ServiceUserManagementController
{
    ...
    public function index($service_user_id = null, Request $request)
    {
        ...
        $mfc_records  = DynamicForm::where('location_id', $this_location_id)
        ...
        if ($request->isMethod('post') && $request->input('filter') == 1) {
            ...
            if ($request->filled('start_date') && $request->filled('end_date')) {
                ...
                $mfc_records->whereBetween('dynamic_form.created_at', [$start, $end]);
            }

            if ($request->filled('keyword')) {
                $keyword = $request->input('keyword');
                $mfc_records->where(function ($query) use ($keyword) {
                    $query->where('title', 'like', "%{$keyword}%")
                        ->orWhere('title', 'like', "%{$keyword}%");
                });
            }

            $mfc_records = $mfc_records->get(); // Get filtered data

        } else {
            

            $today = Carbon::today();
            $mfc_records->whereDate('dynamic_form.created_at', $today);
            // $mfc_records = $mfc_records->filter(function ($record) use ($today) {
            //     return $record->created_at->isSameDay($today);
            // });

            // No filters — get paginated result
            // $mfc_records = $mfc_records->get();
            $mfc_records = $mfc_records->paginate();
        }
